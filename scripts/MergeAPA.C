...
void MergeByTag(TFile* f1, TH2F* hall, const char* tag="hu_orig", double baseline=0, double scale=1){
...
void MergeAPA(const char* rootfile, const char* suffix="-v2.root") {

   //TFile *f1 = TFile::Open("protodune-sim-check.root");
   TFile *f1 = TFile::Open(rootfile);
   TH2F* hu_orig = (TH2F*)f1->Get("hu_orig");
   TH2F* hv_orig = (TH2F*)f1->Get("hv_orig");
   TH2F* hw_orig = (TH2F*)f1->Get("hw_orig");
   ...
   if(!hu_orig){
     ...
     //TFile* fout = new TFile("protodune-sim-check-v3.root","recreate");
     TFile* fout = new TFile(foutname.c_str(),"recreate");
     hu_orig = new TH2F("hu_orig","hu_orig", nbinsx, xmin, xmax, nbinsy, ymin,ymax);
     hv_orig = new TH2F("hv_orig","hv_orig", nbinsx, xmin, xmax, nbinsy, ymin,ymax);
     hw_orig = new TH2F("hw_orig","hw_orig", nbinsx, xmin, xmax, nbinsy, ymin,ymax);
     MergeByTag(f1, hu_orig, "hu_orig");
     MergeByTag(f1, hv_orig, "hv_orig");
     MergeByTag(f1, hw_orig, "hw_orig");
     fout->cd();
     hu_orig->Write();
     hv_orig->Write();
     hw_orig->Write();
